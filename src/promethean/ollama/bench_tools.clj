(ns promethean.ollama.bench-tools
  "Tool registry + OpenAI-style schema + argument validation (spec.alpha).

  You define tools via def-tool. Example:
...
(defn- schema->args-spec [schema]
  (let [properties (:properties schema)
        required (set (map kwish (:required schema)))
        allowed-keys (set (map kwish (keys properties)))
        validators (into {}
                         (map (fn [[key info]]
                                (let [pred (get type->pred (kwish (:type info)) (constantly true))]
                                  {(kwish key) pred})))
                         properties)]
    (if (and (map? schema) (seq properties))
      (s/and
        map?
        (fn [m] (every? #(contains? m %) required))
...
(defmacro def-tool
  ...
  Supports two forms:
  1) Directive DSL (doc/domain/tags/params/impl/bench).
  2) Map-based tool spec with optional implementation.
  ...
  (let [nm (name tool-name)
    ...
    (if (map? first-form)
      (let [tool-map first-form
            argv (second body)
            impl-body (nnext body)
            impl-fn (when (vector? argv)
                      `(fn ~argv ~@impl-body))
            schema (:parameters tool-map)
            args-spec (schema->args-spec schema)
            bench-cases (:bench/cases tool-map)
            bench-map (when (seq bench-cases)
                        {:bench/prompt-templates bench-cases})
            extra-meta (dissoc tool-map :description :parameters :domain :tags :bench/cases)]
        `(def ~tool-name
           (register-tool!
             (merge
               {:name ~nm
                :description ~(:description tool-map)
                :domain ~(:domain tool-map)
                :tags ~(or (:tags tool-map) #{})
                :parameters ~schema
                :args-spec ~args-spec
                :impl ~(or impl-fn `(fn [_#] (throw (ex-info "Tool has no impl" {:tool ~nm}))))}
               ~extra-meta
               ~bench-map))))
      (let [doc-str (or (some #(when (string? %) %) body)
